...
write_content (GMimePart *part, GMimeFormatOptions *options, GMimeStream *stream)
{
	...
	if (part->encoding != g_mime_data_wrapper_get_encoding (part->content)) {
		...
		GMimeStream *content = g_mime_data_wrapper_get_stream (part->content);
		
        /*create string stream*/
        GByteArray *string_array = g_byte_array_new();

        GMimeStream *string_stream = g_mime_stream_mem_new();

        g_mime_stream_mem_set_byte_array(GMIME_STREAM_MEM(string_stream), string_array);

        GMimeStream *filtered_string_stream = g_mime_stream_filter_new(string_stream);

        g_object_unref(string_stream);
		
		if (part->encoding != GMIME_CONTENT_ENCODING_BINARY) {
			filter = g_mime_format_options_create_newline_filter (options, object->ensure_newline);
            g_mime_stream_filter_add((GMimeStreamFilter *)filtered_string_stream, filter);
			g_object_unref (filter);
		}

        /*do decode*/
        if (part->encoding == GMIME_CONTENT_ENCODING_QUOTEDPRINTABLE
            || part->encoding == GMIME_CONTENT_ENCODING_BASE64) {
            filter = g_mime_filter_basic_new(part->encoding, FALSE);
            g_mime_stream_filter_add((GMimeStreamFilter *)filtered_string_stream, filter);
            g_object_unref(filter);
        }
        ...
        */
        ...
        for (guint i = 0; i < object->headers->array->len; i++) {
			GMimeHeader* header = (GMimeHeader *)object->headers->array->pdata[i];
            ...
            if (!g_mime_format_options_is_hidden_header(options, header->name)) {
                ...
                } else if (_stricmp(header->name, "Content-Disposition") == 0) {
					const gchar* header_value = g_mime_header_get_value(header);
					if (header_value) {
						char* begin = strstr(header_value, "filename=");
						if (begin) {
							begin += strlen("filename=");
							...
							attach_filename_utf8 = g_strdup(begin);
							if (attach_filename_utf8) {
								ssize_t namelen = strlen(attach_filename_utf8);
								if (namelen > 0 && attach_filename_utf8[namelen - 1] == '\"') {
									attach_filename_utf8[namelen - 1] = '\0';
								}
							}
						}
					}
		...
		g_mime_stream_flush(filtered_string_stream);
        g_object_unref(filtered_string_stream);
        ...
        if (isText) {
            ...
            if (str && charset[0]) {
                ...
                if (locale && _stricmp(locale, "UTF-8") != 0) {
					...
					const char* utf8 = g_mime_charset_iconv_name("UTF-8");

                    cd = iconv_open(utf8, locale);
                    if (cd != (iconv_t)-1) {
                        dst_buf = g_mime_iconv_strdup(cd, str);
                        iconv_close(cd);
                    } else {
                        dst_buf = g_strdup(str);
                    }
                    ...
                    if (dst_buf) {
                        ...
                        while (nwritten < nread) {
                            ...
                            if ((len = g_mime_stream_write(stream, dst_buf + nwritten, nread - nwritten)) < 0) {
                                break;
                            }
            ...
            if (!is_wrote_stream) {
                ...
                while (nwritten < nread) {
                    ...
                    if ((len = g_mime_stream_write(stream, str + nwritten, nread - nwritten)) < 0) {
                        break;
                    }
        ...
        g_byte_array_free(string_array, TRUE);
