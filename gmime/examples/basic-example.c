...
write_message_to_screen (GMimeMessage *message)
{
	...
	g_mime_object_write_to_stream ((GMimeObject *) message, g_mime_format_options_get_default(), stream);
...
static void
write_message_to_file(GMimeMessage *message, const wchar_t *filePath) {
    if (!message || !filePath) {
        return;
    }
...
int main (int argc, char **argv)
{
    ...
    if (!lpArglist || nArgs < 2) {
		return -1;
    }

    wchar_t attach_dirPath_w[MAX_PATH];
	
	wchar_t* fileOrDir = lpArglist[1];
	if (PathIsDirectoryW(fileOrDir)) {
        WIN32_FIND_DATAW findFileData;
        memset(&findFileData, 0, sizeof(WIN32_FIND_DATAW));

		wchar_t findStr[MAX_PATH];
		_snwprintf_s(findStr, _countof(findStr), _TRUNCATE, L"%s\\*.eml", fileOrDir);

        wchar_t findFilePath[MAX_PATH];

        HANDLE fileHandle = FindFirstFileW(findStr, &findFileData);
        if (INVALID_HANDLE_VALUE != fileHandle) {
            do {
                ...
                if (!(findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                    _snwprintf_s(findFilePath, _countof(findFilePath), _TRUNCATE, L"%s\\%s", fileOrDir, findFileData.cFileName);

					fd = _wopen(findFilePath, O_RDONLY, 0);
					if (fd != -1) {
						...
						const char* to_ptr2 = internet_address_list_to_string(to, options, 0);
						g_mime_format_options_free(options);

						const char* p = g_mime_message_get_subject(message);

						GDateTime* date = g_mime_message_get_date(message);
						long long dateUnixTime = g_date_time_to_unix(date);
						...
						wcsncpy_s(attach_dirPath_w, _countof(attach_dirPath_w), findFilePath, _TRUNCATE);
						wchar_t* begin = wcsrchr(attach_dirPath_w, L'.');
						if (begin) {
							*begin = L'\0';
							wcscat_s(attach_dirPath_w, _countof(attach_dirPath_w), L"-attachFiles\\");
						}

						options = g_mime_format_options_new();
						g_mime_format_options_set_parse_richtext(options, FALSE);
						char* content = g_mime_object_to_string_with_attachfile(attach_dirPath_w, (GMimeObject*)message, options);
						g_free(content);

						g_mime_format_options_set_parse_richtext(options, TRUE);
						char* rich_content = g_mime_object_to_string_with_attachfile(attach_dirPath_w, (GMimeObject*)message, options);
						g_mime_format_options_free(options);
						g_free(rich_content);
        ...
        wcsncpy_s(attach_dirPath_w, _countof(attach_dirPath_w), fileOrDir, _TRUNCATE);
        wchar_t* begin = wcsrchr(attach_dirPath_w, L'.');
        if (begin) {
            *begin = L'\0';
            wcscat_s(attach_dirPath_w, _countof(attach_dirPath_w), L"-attachFiles\\");
