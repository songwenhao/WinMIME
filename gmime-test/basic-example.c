...
int main (int argc, char **argv)
{
    ...
    wchar_t *attach_dirPath_w = NULL;
    gsize attach_dirPath_len = 1024;
    attach_dirPath_w = g_malloc(attach_dirPath_len * sizeof(wchar_t));
    if (attach_dirPath_w) {
        memset(attach_dirPath_w, 0, attach_dirPath_len * sizeof(wchar_t));

        HANDLE fileHandle = (HANDLE)_get_osfhandle(fd);

        GetFinalPathNameByHandleW(fileHandle, attach_dirPath_w, attach_dirPath_len, FILE_NAME_NORMALIZED);
        ...
        swprintf_s(attach_dirPath_w, attach_dirPath_len, L"%s-attachFiles\\", attach_dirPath_w);
